...
void MELScript<ElemType>::CallFunction(const std::string& p_name, const ConfigParamList& params)
{
    ...
    else if (EqualInsensitive(name, "Quantize"))
    {
        ...
        for (auto& pNode : nodes)
        {
            ...
            // Short quantization is the only currently supported
            ...
            shared_ptr<LearnableParameterQuantized<ElemType, short>> pParamNodeQuant(new LearnableParameterQuantized<ElemType, short>(pParamNode, quantizer));
            pNode->CopyTo(pParamNodeQuant, pParamNode->GetName(), CopyNodeFlags::copyNodeValue);
